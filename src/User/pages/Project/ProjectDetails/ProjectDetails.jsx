            ...
            <div key={val.id} className={`${style.projectContainer} ${isArabic ? rtlStyle.projectContainer : ''}`}>
              ...
              {val.projectDetails?.map((item) => (
                  <div key={item.id} className={`${style.projectDetailsSection} ${isArabic ? rtlStyle.projectDetailsSection : ''}`}>
                    <p className={`${style.projectSubject} ${isArabic ? rtlStyle.projectSubject : ''}`}>{item.subjectDetails}</p>
                    <p className={`${style.projectDataDescription} ${isArabic ? rtlStyle.projectDataDescription : ''}`}>{val.data}</p>
                    <p className={`${style.projectDescription} ${isArabic ? rtlStyle.projectDescription : ''}`}>{item.cardDescription}</p>
                    {Object.keys(item.imageGroup).map((imageKey) => (
                        <div key={imageKey} className={style.projectImageGroupItem}>
                          <img src={item.imageGroup[imageKey].url} alt={`Image ${imageKey}`} width="100%" className={style.projectImage} />
