...
export const ProjectStory = ({ children }) => {
    const [project, setProject] = useState([]); // Hook should be inside the component
    const [error, setError] = useState(null); // Also inside the component
    const { i18n } = useTranslation();
    const currentLanguage = i18n.language;
    const languageId = {
        en: 1,
    ...
    useEffect(() => {
        axios.get(`http://localhost:8080/project_card?language_id=${languageId}`)
            .then(res => {
                setProject(res.data);
            })
            .catch(err => {
    ...
    const contextValue = useMemo(() => ({
        project,
        currentLanguage,
        isArabic: currentLanguage === 'ar', // Assuming `isArabic` was intended
    }), [project, currentLanguage]);
